...
const convertJsonToToon = (obj: any, indent: number = 0): string => {
  ...
  if (Array.isArray(obj)) {
    ...
    const items = obj.map((item, index) => {
      const converted = convertJsonToToon(item, nextIndent)
      const itemType = getValueType(item)
      return `${nextIndentStr}└─ [${index}] (${itemType}): ${converted}`
    })
  ...
  if (typeof obj === 'object') {
    ...
    const items = keys.map((key, index) => {
      const value = obj[key]
      const converted = convertJsonToToon(value, nextIndent)
      const valueType = getValueType(value)
      const isLast = index === keys.length - 1
      const prefix = isLast ? '└─' : '├─'
      return `${nextIndentStr}${prefix} ${key} (${valueType}): ${converted}`
    })
